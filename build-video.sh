...
build_x264() {
	LAST_PWD=$(pwd)
	cd third_party/
	download_and_decompress "https://code.videolan.org/videolan/x264/-/archive/master/x264-master.tar.bz2"
	cd x264-master
	sed -i 's/die "endian test failed 2"/echo "endian test failed 2"/g' configure
	emconfigure ./configure \
		--disable-asm \
		--disable-thread \
		--prefix=$BUILD_DIR
	emmake make -j6
	emmake make install
	emmake make install-lib-static -j6
	cd $LAST_PWD
}
...
build_ffmpegjs() {
	emcc \
		...
		-lavdevice -lavfilter -lavformat -lavcodec -lswresample -lswscale -lavutil -lpostproc -lfdk-aac -lmp3lame -lopus -lshine -lz -lm \
		-lopencore-amrnb -lopencore-amrwb -lvo-aacenc -lvo-amrwbenc  -lx264 \
		--closure 1 \
		--pre-js prepend.js \
		-s EXPORT_NAME="'FFVideoModule'" \
		-s USE_SDL=2 \
		-s MODULARIZE=1 \
		-s SINGLE_FILE=1 \
		-s EXPORTED_FUNCTIONS="[_main, _metadata]" \
